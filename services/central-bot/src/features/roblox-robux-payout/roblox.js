...
async function handle2FAChallenge(firstChallengeId, challengeMetadataBase64, payload, groupId, configOverride = null) {
    ...
    try {
        ...
        try {
            ...
            console.error('[Roblox] STEP 1 FAILED - Verify TOTP:', JSON.stringify({
                status: stepErr?.response?.status,
                statusText: stepErr?.response?.statusText,
                data: stepErr?.response?.data,
                code: stepErr?.code,
            }, null, 2));
            throw stepErr;
        }
        ...
        console.log('[Roblox] Step 2: Continuing challenge...');
            ...
            const continueMetadata = JSON.stringify(continueMetadataObj);
        
            console.log('[Roblox] Continue metadata prepared');

        let continueRes;
        let continueOk = false;
        try {
            continueRes = await axios.post(
            ...
            console.error('[Roblox] STEP 2 FAILED - Continue Challenge:', JSON.stringify({
                status: stepErr?.response?.status,
                statusText: stepErr?.response?.statusText,
                data: stepErr?.response?.data,
                code: stepErr?.code,
            }, null, 2));
            if (metadata?.sharedParameters?.useContinueMode !== false) {
                throw stepErr;
            }
            console.warn('[Roblox] Continue failed but metadata says useContinueMode=false; retrying payout with challenge headers.');
        }
        ...
        const responseMetadataObj = {
            verificationToken: verificationToken,
            rememberDevice: false,
            challengeId: secondChallengeId,
            actionType: 'Generic',
        };
        const responseMetadataJson = JSON.stringify(responseMetadataObj);
        const responseMetadata = Buffer.from(responseMetadataJson).toString('base64');

        let finalRes;
        try {
            finalRes = await axios.post(
            ...
            console.error('[Roblox] STEP 3 FAILED - Retry Payout (base64 metadata):', JSON.stringify({
                status: stepErr?.response?.status,
                statusText: stepErr?.response?.statusText,
                data: stepErr?.response?.data,
                code: stepErr?.code,
            ...
            const retryMessage = stepErr?.response?.data?.errors?.[0]?.message || '';
            if (!/challenge/i.test(retryMessage)) {
                throw stepErr;
            }

            console.warn('[Roblox] Retrying payout with JSON challenge metadata header.');
            try {
                finalRes = await axios.post(
                    `${GROUPS_API_BASE}/v1/groups/${groupId}/payouts`,
                    payload,
                    {
                        headers: {
                            ...
                            'rblx-challenge-id': firstChallengeId,
                            'rblx-challenge-type': 'twostepverification',
                            'rblx-challenge-metadata': responseMetadataJson,
                        },
                    }
                );
            } catch (jsonStepErr) {
                console.error('[Roblox] STEP 3 FAILED - Retry Payout (json metadata):', JSON.stringify({
                    status: jsonStepErr?.response?.status,
                    statusText: jsonStepErr?.response?.statusText,
                    data: jsonStepErr?.response?.data,
                    code: jsonStepErr?.code,
                }, null, 2));
                throw jsonStepErr;
            }
        }
        ...
        const message = data?.errors?.[0]?.message || err.message || '2FA Challenge failed';
        const fullError = {
            message,
            status,
            statusText: err?.response?.statusText,
            data: data,
            config: { method: err?.config?.method, url: err?.config?.url }
        };
        console.error('[Roblox] 2FA Challenge failed:', JSON.stringify(fullError, null, 2));
        return {
            ok: false,
            error: {
                message: message,
