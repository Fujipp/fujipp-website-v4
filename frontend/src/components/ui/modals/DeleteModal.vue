    ...
    <Teleport to="body">
        <div :class="$style.backdrop" @click.self="emit('cancel')">
            <section
                ...
                aria-describedby="delete-modal-reason"
            >
                <div :class="$style.heading">
                    <h2 id="delete-modal-title" :class="$style.title">DELETE</h2>
                    <hr :class="$style.divider">
                </div>
                <p id="delete-modal-reason" :class="$style.reason">{{ reason }}</p>
                <div :class="$style.actions">
                    ...
                    <button
                        type="button"
                        :class="[$style.button, $style.deleteButton]"
                        :disabled="disabled"
                        @click="emit('confirm')"
                    >
                        Delete
